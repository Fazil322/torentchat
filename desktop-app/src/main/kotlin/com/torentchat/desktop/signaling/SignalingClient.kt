package com.torentchat.desktop.signaling
...
class SignalingClient(private val relayUrl: String) {
    ...
    suspend fun sendOffer(from: String, to: String, sdp: String) =
        client.post("$relayUrl/v1/signaling/offer") { contentType(ContentType.Application.Json); setBody("""{"from":"$from","to":"$to","sdp":${q(sdp)}}""") }

    suspend fun sendAnswer(from: String, to: String, sdp: String) =
        client.post("$relayUrl/v1/signaling/answer") { contentType(ContentType.Application.Json); setBody("""{"from":"$from","to":"$to","sdp":${q(sdp)}}""") }

    suspend fun sendIce(from: String, to: String, candidate: String) =
        client.post("$relayUrl/v1/signaling/ice") { contentType(ContentType.Application.Json); setBody("""{"from":"$from","to":"$to","candidate":${q(candidate)}}""") }

    suspend fun pollSignaling(peerId: String): PollResponse =
        client.get("$relayUrl/v1/signaling/poll?peerId=$peerId").body()

    suspend fun storePending(from: String, to: String, envelope: String, ttl: Int = 86400) =
        client.post("$relayUrl/v1/pending") { contentType(ContentType.Application.Json); setBody("""{"from":"$from","to":"$to","envelope":${q(envelope)},"ttl":$ttl}""") }

    suspend fun fetchPending(peerId: String): PendingResponse =
        client.get("$relayUrl/v1/pending/$peerId").body()

    suspend fun setPresence(peerId: String, typing: Boolean = false) =
        client.post("$relayUrl/v1/presence") { contentType(ContentType.Application.Json); setBody("""{"peerId":"$peerId","typing":$typing}""") }

    suspend fun getPresence(peerId: String): PresenceResponse =
        client.get("$relayUrl/v1/presence/$peerId").body()

    suspend fun fetchAbConfig(peerId: String): AbConfigResponse =
        client.get("$relayUrl/v1/ab-config/$peerId").body()
