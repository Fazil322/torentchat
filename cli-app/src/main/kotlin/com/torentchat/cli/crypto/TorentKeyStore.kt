...
class TorentKeyStore(
    ...
    private val _id = identityKeyPair
    ...
    private val spks = ConcurrentHashMap<Int, SignedPreKeyRecord>()
    private val sessions = ConcurrentHashMap<SignalProtocolAddress, SessionRecord>()

    override fun getIdentityKeyPair() = _id
    override fun getLocalRegistrationId() = registrationId
    override fun saveIdentity(a: SignalProtocolAddress, k: IdentityKey) = if (trusted[a] != null && trusted[a] != k) { trusted[a] = k; IdentityKeyStore.IdentityChange.REPLACED_EXISTING } else { trusted[a] = k; IdentityKeyStore.IdentityChange.NEW_OR_UNCHANGED }
    ...
    override fun loadSignedPreKey(id: Int) = spks[id] ?: error("SPK $id")
    override fun loadSignedPreKeys() = spks.values.toMutableList()
    override fun storeSignedPreKey(id: Int, r: SignedPreKeyRecord) { spks[id] = r }
    override fun containsSignedPreKey(id: Int) = spks.containsKey(id)
    override fun removeSignedPreKey(id: Int) { spks.remove(id) }
    ...
    override fun markKyberPreKeyUsed(id: Int, s: Int, b: ECPublicKey) {}
